...
//ljw
//
static int mptcp_bytes_seq_show(struct seq_file *seq, void *v)
{
	struct tcp_sock *meta_tp;
	struct net *net = seq->private;
	int i, n = 0;

	seq_printf(seq, "%d", 112233);
    return 0;
}


static int mptcp_bytes_seq_open(struct inode *inode, struct file *file)
{
	return single_open_net(inode, file, mptcp_bytes_seq_show);
}

static const struct file_operations mptcp_bytes_seq_fops = {
	.owner = THIS_MODULE,
	.open = mptcp_bytes_seq_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release_net,
};
...
static int mptcp_pm_init_net(struct net *net)
{
	...
	if (!proc_create("ljw_mptcp_bytes", S_IRUGO, net->proc_net, &mptcp_bytes_seq_fops))
		return -ENOMEM;
